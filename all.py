...
id_current=getInput("current road id")

current_road_type=getInput("current road type",["motorway","rural","town","low" "speed","pedestrian","bicycle","unknown"])
current_road_junctionID=getInput("current junction ID",valueType='i')
speed_max=getInput("max speed",unit="km/h",valueType="f")/3.6
pre_road_type=getInput("predecessor road type",typeList=["road","junction"])
...
successsor_road_type=getInput("successsor road type",typeList=["road","junction"])
...
print>>output_final,"%s|road||||||||name|%s|"%(id_current,road_name)
print>>output_final,"%s|road||||||||length|%f|"%(id_current,road_length)
print>>output_final,"%s|road||||||||id|%s|"%(id_current,id_current)
print>>output_final,"%s|road||||||||junction|%d|"%(id_current,current_road_junctionID)
print>>output_final,"%s|road|link|predecessor||||||elementType|%s|"%(id_current,pre_road_type)
print>>output_final,"%s|road|link|predecessor||||||elementId|%d|"%(id_current,pre_road_id)
print>>output_final,"%s|road|link|predecessor||||||contactPoint|%s|"%(id_current,pre_road_contactPt)


print>>output_final,"%s|road|link|successor||||||elementType|%s|"%(id_current,successsor_road_type)
print>>output_final,"%s|road|link|successor||||||elementId|%d|"%(id_current,successsor_road_id)
print>>output_final,"%s|road|link|successor||||||contactPoint|%s|"%(id_current,successsor_road_contactPt)
# print>>output_final,"%s|road|link|neighbor||||||side|%s|"%(id_current,neighor_road_side)
# print>>output_final,"%s|road|link|neighbor||||||elementId|%d|"%(id_current,neighor_road_id)
# print>>output_final,"%s|road|link|neighbor||||||direction|%s|"%(id_current,neighor_road_direction)
print>>output_final,"%s|road|type|speed||||||max|%d|"%(id_current,speed_max)
print>>output_final,"%s|road|type|speed||||||unit|m/s|"%id_current
print>>output_final,"%s|road|type|||||||s|%f|"%(id_current,0)
print>>output_final,"%s|road|type|||||||type|%s|"%(id_current,current_road_type)
...
while start<size:
	if(len(pointX[start:end])>1):
		...
		# 	print>>output_final,"%s|road|planView|geometry||||||y|%f|"%(id_current,pointY[start])
		# 	print>>output_final,"%s|road|planView|geometry||||||hdg|%f|"%(id_current,hdg)
		# 	print>>output_final,"%s|road|planView|geometry||||||length|%f|"%(id_current,circlelineLength)
		# 	print>>output_final,"%s|road|planView|geometry|arc|||||curvature|%f|"%(id_current,1/para_circle[2])	
		# 	# firstPtX=x[-1]
...
# 	print>>output_final,"%s|road|elevationProfile|elevation||||||s|%f|double precision. Units meters. Non-negative. Start position (s-coordinate)|"%(id_current,0)	
# 	print>>output_final,"%s|road|elevationProfile|elevation||||||a|%f|double precision. Units meters. Positive or negative|"%(id_current,0)	
# 	print>>output_final,"%s|road|elevationProfile|elevation||||||b|%f|double precision. Dimensionless. Positive or negative.|"%(id_current,0)	
# 	print>>output_final,"%s|road|elevationProfile|elevation||||||c|%f|double precision. Units 1/m. Positive or negative|"%(id_current,0)	
# 	print>>output_final,"%s|road|elevationProfile|elevation||||||d|%f|double precision. Units 1/m2. Positive or negative|"%(id_current,0)	
# 	print>>output_final,"%s|road|lateralProfile|superelevation||||||s|%f|double precision. Units meters. Non-negative. Start position (s-coordinate)|"%(id_current,0)	
# 	print>>output_final,"%s|road|lateralProfile|superelevation||||||a|%f|double precision. Units radians. Positive or negative. Superelevation at s=0|"%(id_current,0)	
# 	print>>output_final,"%s|road|lateralProfile|superelevation||||||b|%f|double precision. Units radians/meter. Positive or negative.|"%(id_current,0)	
# 	print>>output_final,"%s|road|lateralProfile|superelevation||||||c|%f|double precision. Units radians/m2. Positive or negative|"%(id_current,0)	
# 	print>>output_final,"%s|road|lateralProfile|superelevation||||||d|%f|double precision. Units radians/m3. Positive or negative|"%(id_current,0)	
...
for i in range(laneSectionNb):
	...
	# print>>output_final,"%s|road|lanes|laneOffset||||||s|%f|"%(id_current,lane_offset_s)
	# print>>output_final,"%s|road|lanes|laneOffset||||||a|%f|"%(id_current,lane_offset_a)
	# print>>output_final,"%s|road|lanes|laneOffset||||||b|%f|"%(id_current,lane_offset_b)
	# print>>output_final,"%s|road|lanes|laneOffset||||||c|%f|"%(id_current,lane_offset_c)
	# print>>output_final,"%s|road|lanes|laneOffset||||||d|%f|"%(id_current,lane_offset_d)
	lane_section_s=sectionS
	lane_section_singleSide=""
	# print(lane_section_s)
	print>>output_final,"%s|road|lanes|laneSection||||||s|%f|"%(id_current,lane_section_s)
	
	if checkSingleSide(sectionLanesInfo[0]):
		lane_section_singleSide="true"
	else:
		lane_section_singleSide="false"
	print>>output_final,"%s|road|lanes|laneSection||||||singleSide|%s|"%(id_current,lane_section_singleSide)
		...
		if allPass==False:
			...
			for i in range(len(parameters)):
				...
				if distantAllLanes[i][0]=="center":
					...
					print>>output_final,"%s|road|lanes|laneSection|center|lane||||type|%s|"%(id_current,center_lane_type)	
					print>>output_final,"%s|road|lanes|laneSection|center|lane||||id|%d|"%(id_current,center_lane_id)	
					print>>output_final,"%s|road|lanes|laneSection|center|lane||||level|%s|"%(id_current,"true")	
					#to add# center_lane_prodecessor_id=raw_input("center lane prodecessor id ",'i')
					center_lane_prodecessor_id=0
					print>>output_final,"%s|road|lanes|laneSection|center|lane|link|predecessor||id|%d|"%(id_current,center_lane_prodecessor_id)	
					#to add# center_lane_successor_id=raw_input("Center lane successor id",'i')
					center_lane_successor_id=0
					print>>output_final,"%s|road|lanes|laneSection|center|lane|link|successor||id|%d|"%(id_current,center_lane_successor_id)	
					...
					if center_lane_roadmark_type!="":
						...
						print>>output_final,"%s|road|lanes|laneSection|center|lane|roadMark|||sOffset|%f|"%(id_current,center_lane_roadmark_offset)	
						print>>output_final,"%s|road|lanes|laneSection|center|lane|roadMark|||type|%s|"%(id_current,center_lane_roadmark_type)	
						center_lane_roadmark_weight=sectionLanesInfo[0][i][4]

						print>>output_final,"%s|road|lanes|laneSection|center|lane|roadMark|||weight|%s|"%(id_current,center_lane_roadmark_weight)	
						
						 #to add#
						center_lane_roadmark_color="standard"
						print>>output_final,"%s|road|lanes|laneSection|center|lane|roadMark|||color|%s|"%(id_current,center_lane_roadmark_color)	
						center_lane_roadmark_material="standard"
						print>>output_final,"%s|road|lanes|laneSection|center|lane|roadMark|||material|%s|"%(id_current,center_lane_roadmark_material)	

						center_lane_roadmark_width=float(sectionLanesInfo[0][i][2])/1000
						print>>output_final,"%s|road|lanes|laneSection|center|lane|roadMark|||width|%f|"%(id_current,center_lane_roadmark_width)
						center_lane_roadmark_laneChange="none"
						print>>output_final,"%s|road|lanes|laneSection|center|lane|roadMark|||laneChange|%s|"%(id_current,center_lane_roadmark_laneChange)
						center_lane_roadmark_height=0
						print>>output_final,"%s|road|lanes|laneSection|center|lane|roadMark|||height|%f|"%(id_current,center_lane_roadmark_height)
			...
			#### right lane
				if distantAllLanes[i][0]=="right":
					...
					print>>output_final,"%s|road|lanes|laneSection|right|lane||||type|%s|"%(id_current,right_lane_type)	
					print>>output_final,"%s|road|lanes|laneSection|right|lane||||id|%d|"%(id_current,right_lane_id)	
					print>>output_final,"%s|road|lanes|laneSection|right|lane||||level|%s|"%(id_current,"true")
					# print>>output_final,"%s|road|lanes|laneSection|right|lane||||level|%s|True = flat. Do not apply superelevation. False=apply crossfall and superelevation|"%(id_current,"true")	
					right_lane_predecessor_id=right_lane_id
					print>>output_final,"%s|road|lanes|laneSection|right|lane|link|predecessor||id|%d|"%(id_current,right_lane_predecessor_id)	
					right_lane_successor_id=right_lane_id
					print>>output_final,"%s|road|lanes|laneSection|right|lane|link|successor||id|%d|"%(id_current,right_lane_successor_id)	
					right_lane_width_sOffset=0
					print>>output_final,"%s|road|lanes|laneSection|right|lane|width|||sOffset|%f|"%(id_current,right_lane_width_sOffset)
					# right_lane_width_a=-float(sectionLanesInfo[0][i][0])
					...
					print>>output_final,"%s|road|lanes|laneSection|right|lane|width|||a|%f|"%(id_current,right_lane_width_a)
					print>>output_final,"%s|road|lanes|laneSection|right|lane|width|||b|%f|"%(id_current,right_lane_width_b)
					print>>output_final,"%s|road|lanes|laneSection|right|lane|width|||c|%f|"%(id_current,right_lane_width_c)
					print>>output_final,"%s|road|lanes|laneSection|right|lane|width|||d|%f|"%(id_current,right_lane_width_d)
					# right_lane_border_sOffset=0
					# print>>output_final,"%s|road|lanes|laneSection|right|lane|border|||sOffset|%f|"%(id_current,right_lane_border_sOffset)
					# #question:what's this
					# right_lane_border_a=0
					# right_lane_border_b=0
					# right_lane_border_c=0
					# right_lane_border_d=0
					# print>>output_final,"%s|road|lanes|laneSection|right|lane|border|||a|%f|"%(id_current,right_lane_border_a)
					# print>>output_final,"%s|road|lanes|laneSection|right|lane|border|||b|%f|"%(id_current,right_lane_border_b)
					# print>>output_final,"%s|road|lanes|laneSection|right|lane|border|||c|%f|"%(id_current,right_lane_border_c)
					# print>>output_final,"%s|road|lanes|laneSection|right|lane|border|||d|%f|"%(id_current,right_lane_border_d)
					...
					if right_lane_roadmark_type!="":
						...
						print>>output_final,"%s|road|lanes|laneSection|right|lane|roadMark|||sOffset|%f|"%(id_current,right_lane_roadmark_offset)	
						print>>output_final,"%s|road|lanes|laneSection|right|lane|roadMark|||type|%s|"%(id_current,right_lane_roadmark_type)	
						right_lane_roadmark_weight=sectionLanesInfo[0][i][4]
						print>>output_final,"%s|road|lanes|laneSection|right|lane|roadMark|||weight|%s|"%(id_current,right_lane_roadmark_weight)	
						
						right_lane_roadmark_color="standard"
						print>>output_final,"%s|road|lanes|laneSection|right|lane|roadMark|||color|%s|"%(id_current,right_lane_roadmark_color)	
						right_lane_roadmark_material="standard"
						print>>output_final,"%s|road|lanes|laneSection|right|lane|roadMark|||material|%s|"%(id_current,right_lane_roadmark_material)	
						right_lane_roadmark_width=float(sectionLanesInfo[0][i][2])/1000
						print>>output_final,"%s|road|lanes|laneSection|right|lane|roadMark|||width|%f|"%(id_current,right_lane_roadmark_width)
						right_lane_roadmark_laneChange="none"
						print>>output_final,"%s|road|lanes|laneSection|right|lane|roadMark|||laneChange|%s|"%(id_current,right_lane_roadmark_laneChange)
						
						right_lane_roadmark_height=0
						print>>output_final,"%s|road|lanes|laneSection|right|lane|roadMark|||height|%f|"%(id_current,right_lane_roadmark_height)
				...
				if distantAllLanes[i][0]=="left":
					...
					print>>output_final,"%s|road|lanes|laneSection|left|lane||||type|%s|"%(id_current,left_lane_type)	
					print>>output_final,"%s|road|lanes|laneSection|left|lane||||id|%d|"%(id_current,left_lane_id)	
					print>>output_final,"%s|road|lanes|laneSection|left|lane||||level|%s|"%(id_current,"true")
					# print>>output_final,"%s|road|lanes|laneSection|right|lane||||level|%s|True = flat. Do not apply superelevation. False=apply crossfall and superelevation|"%(id_current,"true")	
					left_lane_predecessor_id=left_lane_id
					print>>output_final,"%s|road|lanes|laneSection|left|lane|link|predecessor||id|%d|"%(id_current,left_lane_predecessor_id)	
					left_lane_successor_id=left_lane_id
					print>>output_final,"%s|road|lanes|laneSection|left|lane|link|successor||id|%d|"%(id_current,left_lane_successor_id)	
					left_lane_width_sOffset=0
					print>>output_final,"%s|road|lanes|laneSection|left|lane|width|||sOffset|%f|"%(id_current,left_lane_width_sOffset)
					# left_lane_width_a=float(sectionLanesInfo[0][i][0])
					# left_lane_width_b=0
					# left_lane_width_c=0
					# left_lane_width_d=0
					...
					print>>output_final,"%s|road|lanes|laneSection|left|lane|width|||a|%f|"%(id_current,left_lane_width_a)
					print>>output_final,"%s|road|lanes|laneSection|left|lane|width|||b|%f|"%(id_current,left_lane_width_b)
					print>>output_final,"%s|road|lanes|laneSection|left|lane|width|||c|%f|"%(id_current,left_lane_width_c)
					print>>output_final,"%s|road|lanes|laneSection|left|lane|width|||d|%f|"%(id_current,left_lane_width_d)
					# right_lane_border_sOffset=0
					# print>>output_final,"%s|road|lanes|laneSection|right|lane|border|||sOffset|%f|"%(id_current,right_lane_border_sOffset)
					# #question:what's this
					# right_lane_border_a=0
					...
					if left_lane_roadmark_type!="":
						...
						print>>output_final,"%s|road|lanes|laneSection|left|lane|roadMark|||sOffset|%f|"%(id_current,left_lane_roadmark_offset)	
						print>>output_final,"%s|road|lanes|laneSection|left|lane|roadMark|||type|%s|"%(id_current,left_lane_roadmark_type)	
						left_lane_roadmark_weight=sectionLanesInfo[0][i][4]
						print>>output_final,"%s|road|lanes|laneSection|left|lane|roadMark|||weight|%s|"%(id_current,left_lane_roadmark_weight)	
						
						left_lane_roadmark_color="standard"
						print>>output_final,"%s|road|lanes|laneSection|left|lane|roadMark|||color|%s|"%(id_current,left_lane_roadmark_color)	
						left_lane_roadmark_material="standard"
						print>>output_final,"%s|road|lanes|laneSection|left|lane|roadMark|||material|%s|"%(id_current,left_lane_roadmark_material)	
						left_lane_roadmark_width=float(sectionLanesInfo[0][i][2])/1000
						print>>output_final,"%s|road|lanes|laneSection|left|lane|roadMark|||width|%f|"%(id_current,left_lane_roadmark_width)
						left_lane_roadmark_laneChange="none"
						print>>output_final,"%s|road|lanes|laneSection|left|lane|roadMark|||laneChange|%s|"%(id_current,left_lane_roadmark_laneChange)
						
						left_lane_roadmark_height=0
						print>>output_final,"%s|road|lanes|laneSection|left|lane|roadMark|||height|%f|"%(id_current,left_lane_roadmark_height)
						#don't have roadmark type
						#don't have material/visibility/speed/access/height/rule
			...
			if endPos<sectionSize+1:
				...
				endPos=sectionSize+1
				print>>output_final,"%s|road|lanes|laneSection||||||s|%f|"%(id_current,interS)
				print>>output_final,"%s|road|lanes|laneSection||||||singleSide|%s|"%(id_current,lane_section_singleSide)
				allPass=True
	sectionS=sectionS+s		
	# else:
...
# 	print>>output_final,"%s|road|objects|tunnel||||||name|%s|"%(id_current,tunnel_name)
# 	tunnel_s=float(raw_input("tunnel s : ") or 0)
# 	print>>output_final,"%s|road|objects|tunnel||||||s|%f|"%(id_current,tunnel_s)
# 	tunnel_length=float(raw_input("tunnel length : ") or 0)
# 	print>>output_final,"%s|road|objects|tunnel||||||length|%f|"%(id_current,tunnel_length)
# 	tunnel_id=int(raw_input("tunnel id : ") or 0)
# 	print>>output_final,"%s|road|objects|tunnel||||||id|%d|"%(id_current,tunnel_id)
# 	tunnel_lighting=float(raw_input("tunnel lighting : ") or 0)
# 	print>>output_final,"%s|road|objects|tunnel||||||lighting|%f|"%(id_current,tunnel_lighting)
# 	tunnel_daylight=float(raw_input("tunnel daylight : ") or 0)
# 	print>>output_final,"%s|road|objects|tunnel||||||daylight|%f|"%(id_current,tunnel_daylight)
# 	tunnel_type=raw_input("tunnel type : standard(s)/underpass(u) ") or "s"
# 	if tunnel_type=='u':
# 		tunnel_type="underpass"
# 	else:
# 		tunnel_type="standard"
# 	print>>output_final,"%s|road|objects|tunnel||||||type|%s|"%(id_current,tunnel_type)
# 	tunnel_fromLane=int(raw_input("tunnel from lane id : ") or 1)
# 	print>>output_final,"%s|road|objects|tunnel|validity|||||fromLane|%d|"%(id_current,tunnel_fromLane)
# 	tunnel_toLane=int(raw_input("tunnel to lane id : ") or 2)
# 	print>>output_final,"%s|road|objects|tunnel|validity|||||toLane|%d|"%(id_current,tunnel_toLane)
# hasBridge=raw_input("is there bridge?(y/n)") or "y"
# if hasBridge=="y":
# 	bridge_name=raw_input("bridge name : ") or "test bridge"
# 	print>>output_final,"%s|road|objects|bridge||||||name|%s|"%(id_current,bridge_name)
# 	bridge_s=float(raw_input("bridge s : ") or 0)
# 	print>>output_final,"%s|road|objects|bridge||||||s|%f|"%(id_current,bridge_s)
# 	bridge_length=float(raw_input("bridge length : ") or 0)
# 	print>>output_final,"%s|road|objects|bridge||||||length|%f|"%(id_current,bridge_length)
# 	bridge_id=int(raw_input("bridge id : ") or 0)
# 	print>>output_final,"%s|road|objects|bridge||||||id|%d|"%(id_current,bridge_id)
